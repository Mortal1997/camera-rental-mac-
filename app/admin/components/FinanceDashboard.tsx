...
import { EmptyState, MetricCard, PageHeader, PrimaryButton, SecondaryButton, SectionHeader, StatBadge, SurfaceCard, TableHead, TableShell, Td, TextInput, Th, Tr } from './ui';
    ...
    <div className="flex flex-col gap-6">
      ...
      <SurfaceCard>
        <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
          <div className="grid gap-4 md:grid-cols-2 xl:min-w-[420px]">
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <MetricCard icon={BarChart3} iconClassName="bg-emerald-50 text-emerald-600" label="所选期间总营业额" value={formatCurrency(report.totalRevenue)} valueClassName="text-emerald-600" />
        <MetricCard icon={BarChart3} iconClassName="bg-blue-50 text-blue-600" label="已完成订单总数" value={report.totalOrders} />
        <MetricCard icon={BarChart3} iconClassName="bg-indigo-50 text-indigo-600" label="客单价" value={formatCurrency(averageOrderValue)} />
      </div>
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {report.monthlySummary.length === 0 ? (
            ...
            report.monthlySummary.map((item) => (
              <div key={item.month} className="rounded-2xl border border-slate-100 bg-slate-50 p-5">
                <div className="flex items-center justify-between">
                  <p className="text-sm font-semibold text-slate-900">{formatMonthLabel(item.month)}</p>
                  <StatBadge>{item.orderCount} 单</StatBadge>
                </div>
                <p className="mt-4 text-2xl font-bold tracking-tight text-emerald-600">{formatCurrency(item.totalRevenue)}</p>
              </div>
            ))
          )}
        </div>
              ...
              <tbody>
                {report.orders.map((order) => (
                  <Tr key={order.id}>
                    ...
                    <Td className="font-semibold text-emerald-600">{formatCurrency(Number(order.total_price || 0))}</Td>
                  </Tr>
                ))}
              </tbody>
            </table>
